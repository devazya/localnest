...
const SLIDESHOW_IMAGES = [
  'https://images.unsplash.com/photo-1596176530529-78163a4f7af2?w=800&q=85', // Vidhana Soudha
  'https://images.unsplash.com/photo-1600100397608-5e5aca5c7d22?w=800&q=85', // Cubbon Park
  'https://images.unsplash.com/photo-1582719508461-905c673771fd?w=800&q=85', // MG Road / city
  'https://images.unsplash.com/photo-1570168007204-dfb528c6958f?w=800&q=85', // UB City
  'https://images.unsplash.com/photo-1533929736458-ca588d08c8be?w=800&q=85', // Church Street
  'https://images.unsplash.com/photo-1587474260584-136574528ed5?w=800&q=85', // Lalbagh
];
...
/* ─────────────────────────────────────
   COMMUNITY BUZZ section data
───────────────────────────────────── */
const BUZZ_POSTS = [
  { id:1, icon:'🐕', title:'Lost Brown Labrador',     sub:'Last seen near Cubbon Park. Please contact Ramya if sighted.',  tag:'LOST & FOUND', tagColor:'#EF4444', time:'4h ago', author:'Ramya' },
  { id:2, icon:'💧', title:'Water Supply Disruption', sub:'No supply from 6AM–10AM tomorrow. Store water tonight.',        tag:'NOTICE',       tagColor:'#3B82F6', time:'2h ago', author:'Green Sector 12' },
  { id:3, icon:'🚧', title:'Road Closure – 5th Main', sub:'Road blocked for 3 days. Use the bypass via 8th Cross.',         tag:'ALERT',        tagColor:'#D97706', time:'3h ago', author:'Green Sector' },
  { id:4, icon:'🩸', title:'Urgent: O+ Blood Donor',  sub:'Emergency at Manipal Hospital. Please contact Ravi: 9876500001.', tag:'URGENT',      tagColor:'#EF4444', time:'1h ago', author:'Ravi K.' },
  { id:5, icon:'📋', title:'Apartment AGM – Block C',  sub:'Meeting this Saturday 10AM at Clubhouse. All residents welcome.', tag:'NOTICE',       tagColor:'#7C3AED', time:'6h ago', author:'Block C Admin' },
];

/* ─────────────────────────────────────
   NEARBY BUSINESSES section data
───────────────────────────────────── */
...
const UPCOMING_EVENTS = [
  {
    id:1, icon:'🎵', bg:'linear-gradient(135deg,#7C3AED,#4F1FA3)',
    date:'SAT 28 JUN',
    title:'Live Music Night',
    time:'7:00 PM Onwards',
    location:'Green Arena, 1.2 km',
    avatars:['#6D4AFF','#EC4899','#10B981'],
    count:24,
  },
  {
    id:2, icon:'⚽', bg:'linear-gradient(135deg,#059669,#064E3B)',
    date:'SUN 29 JUN',
  ...
  {
    id:3, icon:'🎨', bg:'linear-gradient(135deg,#D97706,#92400E)',
    date:'FRI 04 JUL',
    title:'Art & Craft Workshop',
    time:'3:00 PM Onwards',
    location:'Green Hub, 800 m',
    avatars:['#EF4444','#10B981','#6D4AFF'],
    count:12,
  },
  {
    id:4, icon:'🎤', bg:'linear-gradient(135deg,#DC2626,#7F1D1D)',
    date:'SAT 05 JUL',
    title:'Stand-up Comedy',
    time:'6:00 PM Onwards',
    location:'Open Air Theatre',
    avatars:['#6D4AFF','#10B981','#EC4899'],
    count:32,
  },
];
...
function HomeHeader({ onNavigate, children }) {
  ...
  return (
    <div style={{ position:'relative', overflow:'hidden', paddingBottom:0 }}>
      {/* ── Slideshow Background — fills entire hero including Local Pulse ── */}
      <div style={{ position:'absolute', inset:0, zIndex:0 }}>
        {/* Current image — 6px blur so city buildings stay clearly visible */}
        <img
          ...
          style={{
            ...
            objectFit:'cover', filter:'blur(6px)',
            transform:'scale(1.04)',
            opacity: fading ? 0 : 1,
            transition:`opacity ${800}ms ease`,
          }}
        />
        {next !== null && (
          <img
        ...
        {/* Light dark overlay — ~22% so city colours remain vibrant */}
        <div style={{ position:'absolute', inset:0, background:'rgba(0,0,0,0.22)' }} />
        {/* Very subtle purple depth */}
      ...
      <div style={{ position:'relative', zIndex:1 }}>
        ...
        {/* Layer 2: Greeting + Weather side by side */}
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:12, padding:'20px 20px 0' }}>
          ...
          {/* Weather glass pill — compact, single line */}
        ...
        {/* Layer 3: Glass Local Pulse card — floats inside hero, bottom flush with white section */}
        ...
        <div style={{ display:'flex', margin:'8px 20px 0', background:'#F3F0FF', borderRadius:14, padding:4 }}>
          {[{id:'compare',label:'🚗 Compare Rides'},{id:'community',label:'🤝 Community Ride'}].map(t => (
            <button
              ...
              style={{ flex:1, padding:'9px 0', border:'none', borderRadius:10, fontSize:13, fontWeight:600, cursor:'pointer', fontFamily:'var(--font-sans)', transition:'all 180ms ease', background:tab===t.id?'#6D4AFF':'transparent', color:tab===t.id?'#fff':'#7C6FCD' }}
            >{t.label}</button>
          ))}
        </div>
          ...
          <div style={{ padding:'20px 20px 0' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10, background:'#F8F7FF', border:'1.5px solid #EDE9FE', borderRadius:14, padding:'12px 14px', marginBottom:16 }}>
              ...
              <input
                ...
                style={{ flex:1, border:'none', background:'transparent', fontSize:14, fontWeight:500, color:'#0D0820', fontFamily:'var(--font-sans)', outline:'none' }}
              />
              {destination && (
            ...
            {searched ? (
              <>
                  ...
                  {Object.entries(RIDE_PRICES).map(([provider, rides]) => (
                    <div key={provider} style={{ background:'#F8F7FF', borderRadius:16, padding:'14px 16px', border:'1px solid #EDE9FE' }}>
                      <div style={{ fontSize:13, fontWeight:700, color:'#0D0820', marginBottom:10, textTransform:'capitalize', display:'flex', alignItems:'center', gap:6 }}>
                        <span style={{ width:10, height:10, borderRadius:'50%', background: provider==='uber'?'#000':provider==='ola'?'#FFCC00':'#F5A623', display:'inline-block' }} />
                        {provider}
                      </div>
                      {rides.map(r => (
                ...
                {['Uber','Ola','Rapido'].map(p => (
                  <button key={p} onClick={() => window.open(p==='Uber'?'uber://':p==='Ola'?'olacabs://':'rapido://', '_blank')}
                    style={{ display:'block', width:'100%', marginBottom:10, padding:'14px', background:'linear-gradient(135deg,#6D4AFF,#8B5CF6)', border:'none', borderRadius:14, color:'#fff', fontSize:14, fontWeight:700, cursor:'pointer', fontFamily:'var(--font-sans)' }}
                  >Book on {p} →</button>
                ))}
              </>
            ) : (
            ...
            {[
              ...
              <div key={field.key} style={{ marginBottom:14 }}>
                ...
                <input
                  ...
                  style={{ width:'100%', boxSizing:'border-box', background:'#F8F7FF', border:'1.5px solid #EDE9FE', borderRadius:14, padding:'12px 14px', fontSize:14, color:'#0D0820', fontFamily:'var(--font-sans)', outline:'none' }}
                />
              </div>
            ))}
              ...
              <div style={{ display:'flex', gap:8 }}>
                {['Any','Women only','Men only'].map(g => (
                  <button key={g} onClick={() => updateForm('gender',g)}
                    style={{ padding:'8px 14px', borderRadius:99, border:'1.5px solid', borderColor:form.gender===g?'#6D4AFF':'#EDE9FE', background:form.gender===g?'#EDE9FE':'#fff', color:form.gender===g?'#6D4AFF':'#9CA3AF', fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:'var(--font-sans)' }}
                  >{g}</button>
                ))}
              </div>
            </div>
...
function NetworkCard({ onNavigate, liveActivity }) {
  return (
    <div style={{
      ...
      borderRadius:26,
      overflow:'hidden',
      boxShadow:'0 18px 45px rgba(70,40,180,0.10), 0 2px 8px rgba(0,0,0,0.06)',
    }}>
      {/* Card header */}
      ...
      <div style={{ fontSize:13, color:'rgba(255,255,255,0.85)', fontWeight:500 }}>Live community activity</div>
        ...
        <div style={{ background:'rgba(255,255,255,0.22)', backdropFilter:'blur(8px)', WebkitBackdropFilter:'blur(8px)', border:'1px solid rgba(255,255,255,0.25)', borderRadius:99, padding:'5px 14px', display:'flex', alignItems:'center', gap:5, boxShadow:'0 2px 8px rgba(0,0,0,0.10)' }}>
          ...
          <span style={{ fontSize:12, fontWeight:700, color:'#fff' }}>186 online</span>
        </div>
      </div>
...
function RideHub({ onOpen }) {
  ...
  /* Proper motorbike side-view for Rapido */
  const RapidoBike = () => (
    <svg width="72" height="42" viewBox="0 0 72 42" fill="none">
      {/* shadows */}
      <ellipse cx="36" cy="40" rx="24" ry="2.5" fill="rgba(0,0,0,0.10)"/>
      {/* rear wheel */}
      <circle cx="14" cy="30" r="10" fill="#78350F"/>
      <circle cx="14" cy="30" r="6.5" fill="#92400E"/>
      <circle cx="14" cy="30" r="3" fill="#F59E0B"/>
      <circle cx="14" cy="30" r="1.2" fill="#FEF3C7"/>
      {/* front wheel */}
      <circle cx="56" cy="30" r="10" fill="#78350F"/>
      <circle cx="56" cy="30" r="6.5" fill="#92400E"/>
      <circle cx="56" cy="30" r="3" fill="#F59E0B"/>
      <circle cx="56" cy="30" r="1.2" fill="#FEF3C7"/>
      {/* chain / swingarm */}
      ...
      <path d="M28 14 C32 8 40 7 48 11 L55 17 L50 20 L34 18 Z" fill="#FBBF24"/>
      {/* fuel tank */}
      ...
      <line x1="53" y1="18" x2="53" y2="28" stroke="#B45309" strokeWidth="1.5" strokeLinecap="round"/>
      {/* handlebar */}
      <line x1="50" y1="14" x2="62" y2="12" stroke="#78350F" strokeWidth="2.5" strokeLinecap="round"/>
      <circle cx="62" cy="12" r="2.5" fill="#1A1A1A"/>
      <circle cx="50" cy="14" r="2" fill="#1A1A1A"/>
      {/* headlight */}
      <ellipse cx="61" cy="20" rx="4" ry="3" fill="#FEF3C7"/>
      <ellipse cx="61" cy="20" rx="2.5" ry="2" fill="#FDE68A"/>
      {/* exhaust */}
  ...
  return (
    <div style={{
      ...
      borderRadius:24,
      overflow:'hidden',
      /* Full-width purple-to-white gradient so left blends into right */
      background:'linear-gradient(105deg, #ffffff 0%, #ffffff 52%, #6D4AFF 52%, #5B3FE0 100%)',
      boxShadow:'0 2px 0 rgba(109,74,255,0.06), 0 14px 44px rgba(109,74,255,0.16)',
    }}>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', padding:'18px 18px 14px' }}>
        ...
        <motion.button
          whileTap={{ scale:0.95 }}
          onClick={onOpen}
          style={{ display:'flex', alignItems:'center', gap:5, background:'#ffffff', border:'1.5px solid #6D4AFF', borderRadius:99, padding:'8px 14px', color:'#6D4AFF', fontSize:12, fontWeight:700, cursor:'pointer', flexShrink:0, fontFamily:'var(--font-sans)', boxShadow:'0 2px 10px rgba(109,74,255,0.14)', whiteSpace:'nowrap' }}
        >
      ...
      {/* ── Body: provider chips (left) blend into city scene (right) ── */}
      <div style={{ display:'flex', alignItems:'stretch', minHeight:170, position:'relative' }}>

        {/* LEFT — provider chips on white, with a gradient fade to purple on right edge */}
        <div style={{
          flex:1,
          display:'flex',
          ...
          /* Fade the white area into the purple city on the right */
          background:'linear-gradient(90deg, #ffffff 60%, rgba(109,74,255,0.0) 100%)',
          position:'relative',
          zIndex:1,
        }}>
          {PROVIDERS.map(p => (
        ...
        {/* RIGHT — purple city animation, bleeds from centre to edge */}
        <div style={{
          width:118,
          flexShrink:0,
          position:'relative',
          overflow:'hidden',
          /* fade-in from white on left edge to solid purple */
          background:'transparent',
        }}>
          {/* Gradient bridge: white → purple */}
          <div style={{
            position:'absolute', inset:0, zIndex:1,
            background:'linear-gradient(90deg, #ffffff 0%, rgba(109,74,255,0) 35%)',
            pointerEvents:'none',
          }}/>
          ...
          >
            ...
            {/* Windows */}
            {[
              [5,85],[5,97],[5,109],[12,85],[12,97],[12,109],
              [27,57],[27,69],[27,81],[27,93],[35,57],[35,69],[35,81],
              [53,77],[53,89],[53,101],[59,77],[59,89],
              [71,46],[71,58],[71,70],[71,82],[80,46],[80,58],[80,70],[80,82],
              [95,73],[95,85],[95,97],[103,73],[103,85],[103,97],
            ].map(([x,y],i) => (
            ...
            <rect x="6"  y="170" width="12" height="2" rx="1" fill="rgba(255,255,255,0.22)"/>
            <rect x="28" y="170" width="12" height="2" rx="1" fill="rgba(255,255,255,0.22)"/>
            <rect x="50" y="170" width="12" height="2" rx="1" fill="rgba(255,255,255,0.22)"/>
            <rect x="72" y="170" width="12" height="2" rx="1" fill="rgba(255,255,255,0.22)"/>
            <rect x="94" y="170" width="12" height="2" rx="1" fill="rgba(255,255,255,0.22)"/>
            ...
            <g>
              <animateTransform
                ...
                type="translate" values="-50 0;170 0" dur="3.8s" repeatCount="indefinite"
                calcMode="linear"
              />
      ...
      <div style={{ padding:'12px 18px 18px', background:'#ffffff' }}>
        <motion.button
          ...
          style={{
            ...
            boxShadow:'0 4px 0 rgba(80,40,180,0.35), 0 8px 24px rgba(109,74,255,0.30)',
          }}
        >
          Compare Ride
        </motion.button>
...
export default function Home({ onNavigate }) {
  ...
  return (
    <div style={{ background:'#ffffff', minHeight:'100vh', paddingBottom:'calc(var(--bottom-nav-h) + var(--safe-bottom) + 16px)' }}>
      ...
      {/* ── Hero: Header + Greeting + Weather + Glass Local Pulse all on one slideshow ── */}
          ...
          <div style={{ display:'flex', gap:14, overflowX:'auto', paddingLeft:20, paddingRight:20, paddingBottom:8, background:'#ffffff', scrollbarWidth:'none', WebkitOverflowScrolling:'touch' }}>
            {LIVE_UPDATES.map(item => (
              <motion.button
                ...
                style={{ flexShrink:0, width:210, background:'#ffffff', border:'none', borderRadius:22, padding:'16px', boxShadow:'0 8px 28px rgba(109,74,255,0.10), 0 2px 8px rgba(109,74,255,0.06)', cursor:'pointer', textAlign:'left', display:'flex', flexDirection:'column', gap:12 }}
              >
          ...
          <div style={{ display:'flex', gap:14, overflowX:'auto', paddingLeft:20, paddingRight:20, paddingBottom:8, background:'#ffffff', scrollbarWidth:'none', WebkitOverflowScrolling:'touch' }}>
            {DEALS.map(deal => (
              <motion.button
                key={deal.id}
                whileTap={{ scale:0.97 }}
                onClick={() => onNavigate(deal.route)}
                style={{ flexShrink:0, width:178, background:'#ffffff', border:'none', borderRadius:24, overflow:'hidden', boxShadow:'0 8px 28px rgba(109,74,255,0.10), 0 2px 8px rgba(109,74,255,0.05)', cursor:'pointer', textAlign:'left', display:'flex', flexDirection:'column' }}
              >
                {/* Image / color area */}
          ...
          <div style={{ display:'flex', gap:14, overflowX:'auto', paddingLeft:20, paddingRight:20, paddingBottom:8, background:'#ffffff', scrollbarWidth:'none', WebkitOverflowScrolling:'touch' }}>
            {UPCOMING_EVENTS.map(ev => (
              <motion.button
                ...
                style={{ flexShrink:0, width:210, background:'#ffffff', border:'none', borderRadius:24, overflow:'hidden', boxShadow:'0 8px 28px rgba(109,74,255,0.10), 0 2px 8px rgba(109,74,255,0.05)', cursor:'pointer', textAlign:'left', display:'flex', flexDirection:'column' }}
              >
                {/* Cover image */}
                <div style={{ height:128, background:ev.bg, display:'flex', alignItems:'center', justifyContent:'center', position:'relative' }}>
                  ...
                  {/* Date badge - split day/date like reference */}
                  <div style={{ position:'absolute', top:10, left:10, background:'rgba(0,0,0,0.55)', backdropFilter:'blur(6px)', borderRadius:12, padding:'5px 10px', display:'flex', flexDirection:'column', alignItems:'center' }}>
                    <div style={{ fontSize:8, fontWeight:800, color:'rgba(255,255,255,0.8)', letterSpacing:0.8, lineHeight:1.2 }}>{ev.date.split(' ')[0]}</div>
                    <div style={{ fontSize:16, fontWeight:900, color:'#fff', lineHeight:1.1 }}>{ev.date.split(' ')[1]}</div>
                    <div style={{ fontSize:8, fontWeight:700, color:'rgba(255,255,255,0.8)', letterSpacing:0.5 }}>{ev.date.split(' ')[2]}</div>
                  </div>
                  {/* Going count badge */}
                  ...
                  <div style={{ display:'flex', alignItems:'center', gap:8, marginTop:12 }}>
                    <div style={{ display:'flex' }}>
                      {ev.avatars.map((color, i) => (
                        <div key={i} style={{ width:26, height:26, borderRadius:'50%', background:color, border:'2px solid #fff', marginLeft: i > 0 ? -10 : 0 }} />
                      ))}
                    </div>
                    <span style={{ fontSize:11, color:'#6D4AFF', fontWeight:700 }}>+{ev.count}</span>
                  </div>
                </div>
              </motion.button>
            ))}
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:14, padding:'0 20px' }}>
            {BUZZ_POSTS.map(post => (
              <motion.button
                ...
                style={{ display:'flex', alignItems:'center', gap:14, background:'#ffffff', border:'none', borderRadius:20, padding:'14px 16px', boxShadow:'0 6px 24px rgba(109,74,255,0.08), 0 2px 6px rgba(109,74,255,0.04)', cursor:'pointer', textAlign:'left', width:'100%', position:'relative', overflow:'hidden' }}
              >
                {/* Left color bar */}
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <div style={{ fontSize:13, fontWeight:700, color:'#0D0820', lineHeight:1.3, marginBottom:3 }}>{post.title}</div>
                  <div style={{ fontSize:11, color:'#6B7280', lineHeight:1.4 }}>{post.sub}</div>
                </div>
              </motion.button>
            ))}
          </div>
          ...
          <div style={{ display:'flex', gap:14, overflowX:'auto', paddingLeft:20, paddingRight:20, paddingBottom:8, background:'#ffffff', scrollbarWidth:'none', WebkitOverflowScrolling:'touch' }}>
            {NEARBY_BUSINESSES.map(biz => (
              <motion.button
                key={biz.id}
                whileTap={{ scale:0.97 }}
                onClick={() => onNavigate(biz.route)}
                style={{ flexShrink:0, width:182, background:'#ffffff', border:'none', borderRadius:24, overflow:'hidden', boxShadow:'0 8px 28px rgba(109,74,255,0.10), 0 2px 8px rgba(109,74,255,0.05)', cursor:'pointer', textAlign:'left', display:'flex', flexDirection:'column' }}
              >
                <div style={{ height:108, background:biz.color, display:'flex', alignItems:'center', justifyContent:'center', position:'relative' }}>
                  ...
                  {biz.sponsored && (
                    <div style={{ position:'absolute', top:10, right:10, background:'rgba(0,0,0,0.45)', borderRadius:99, padding:'3px 10px', fontSize:9.5, fontWeight:700, color:'#fff' }}>Sponsored</div>
                  )}
          ...
          <div style={{ display:'flex', flexWrap:'wrap', gap:12, padding:'0 20px' }}>
            {COMMUNITIES.map(c => (
              <motion.button
                ...
                whileTap={{ scale:0.95 }}
                onClick={() => onNavigate('community')}
                style={{ background:c.color, border:'none', borderRadius:99, padding:'11px 20px', fontSize:13, fontWeight:700, color:c.text, cursor:'pointer', fontFamily:'var(--font-sans)', boxShadow:'0 4px 12px rgba(109,74,255,0.10)' }}
